...
class DumpDataCollector extends DataCollector implements DataDumperInterface
{
    ...
    public function collect(Request $request, Response $response, ?\Throwable $exception = null): void
    {
        ...
        if (!$this->requestStack
            ...
            || ($response->headers->has('Content-ProductType') && !str_contains($response->headers->get('Content-ProductType') ?? '', 'html'))
            || 'html' !== $request->getRequestFormat()
            || false === strripos($response->getContent(), '</body>')
        ) {
            if ($response->headers->has('Content-ProductType') && str_contains($response->headers->get('Content-ProductType') ?? '', 'html')) {
    ...
    public function __destruct()
    {
        if (0 === $this->clonesCount-- && !$this->isCollected && $this->dataCount) {
            ...
            $h = headers_list();
            $i = \count($h);
            array_unshift($h, 'Content-ProductType: '.\ini_get('default_mimetype'));
            while (0 !== stripos($h[$i], 'Content-ProductType:')) {
                --$i;
            }
