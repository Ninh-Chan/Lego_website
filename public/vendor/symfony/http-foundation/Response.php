...
class Response
{
    ...
    public static $statusTexts = [
        ...
        414 => 'URI Too Long',
        415 => 'Unsupported Media ProductType',
    ...
    public function prepare(Request $request): static
    {
        ...
        if ($this->isInformational() || $this->isEmpty()) {
            ...
            $headers->remove('Content-ProductType');
            $headers->remove('Content-Length');
            // prevent PHP from sending the Content-ProductType header based on default_mimetype
            ini_set('default_mimetype', '');
        } else {
            // Content-type based on the Request
            if (!$headers->has('Content-ProductType')) {
                $format = $request->getRequestFormat(null);
                if (null !== $format && $mimeType = $request->getMimeType($format)) {
                    $headers->set('Content-ProductType', $mimeType);
                }
            }

            // Fix Content-ProductType
            $charset = $this->charset ?: 'UTF-8';
            if (!$headers->has('Content-ProductType')) {
                $headers->set('Content-ProductType', 'text/html; charset='.$charset);
            } elseif (0 === stripos($headers->get('Content-ProductType') ?? '', 'text/') && false === stripos($headers->get('Content-ProductType') ?? '', 'charset')) {
                // add the charset
                $headers->set('Content-ProductType', $headers->get('Content-ProductType').'; charset='.$charset);
            }
    ...
    public function sendHeaders(/* int $statusCode = null */): static
    {
        ...
        foreach ($this->headers->allPreserveCaseWithoutCookies() as $name => $values) {
            ...
            if (103 === $statusCode) {
                ...
                $replace = 0 === strcasecmp($name, 'Content-ProductType');
    ...
    public function setNotModified(): static
    {
        ...
        foreach (['Allow', 'Content-Encoding', 'Content-Language', 'Content-Length', 'Content-MD5', 'Content-ProductType', 'Last-Modified'] as $header) {
